    ...
    def __call__(
        ...
        **kwargs,
        ...
        for prompts in prompt_list:
            # encode every text feature
            prompts = prompts if isinstance(prompts, list) else [prompts] * batch_size
            if negative_prompt is not None:
                negative_prompt = negative_prompt if isinstance(negative_prompt, list) else [negative_prompt] * batch_size 
            split_text_embeddings = self._encode_prompt(
                prompts, device, num_videos_per_prompt, do_classifier_free_guidance, negative_prompt
            )
            text_embeddings_list.append(split_text_embeddings)
        ...
        if use_frameinit:
            current_diffuse_timestep = frameinit_noise_level # diffuse to t noise level: 850
            ...
            for i, t in enumerate(timesteps):
                ...
                if first_frame_latents is not None:
                    ...
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings,
                                           ...
                                           masks=modify_masks,text_embeddings_list=text_embeddings_list,
