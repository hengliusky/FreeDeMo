...
class ConditionalAnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # first_hidden,text_embeddings_list = show_wise_hidden(masks=masks,image_path=first_frame_path,device=latents.device,
        #                                                                vae=self.vae,prompt=prompt,negative_prompt=negative_prompt,img_transform=img_transform,
        #                                                                encode_prompt=self._encode_prompt)
        
        if use_frameinit: #True
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance is None:
                    ...
                    latent_model_input = torch.cat([latents] * 2) # torch.Size([2, 4, 15, 32, 32])
                elif do_classifier_free_guidance == "both":
                ...
                if first_frame_latents is not None:
                    ...
                    first_frame_latents_input = first_frame_latents_input.unsqueeze(2) # torch.Size([2, 4, 1, 32, 32])
                    
                    # 分别进行预测
                    # first_hidden,text_embeddings_list=show_wise_index_hidden(masks,first_frame_latents_input,index_list,text_embeddings)
                    first_hidden,text_embeddings_list = show_wise_hidden(masks=masks,image_path=first_frame_path,device=first_frame_latents_input.device,
                                                                       vae=self.vae,prompt=prompt,negative_prompt=negative_prompt,img_transform=img_transform,
                                                                       encode_prompt=self._encode_prompt)

                    # for first_frame_latent , text_embedding in zip(first_hidden,text_embeddings_list): 
                    first_frame_latent= first_hidden[0]
                    text_embedding = text_embeddings_list[0]
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embedding,
                                           first_frame_latents=first_frame_latent, frame_stride=frame_stride,).sample.to(dtype=latents_dtype)
...
def show_wise_index_hidden(masks,first_hidden,index_list,text_embeddings):
    if masks is not None :
        masks = masks.float()
        len_masks=len(masks)
        
        h = w = first_hidden.shape[-1] 
        _,_,H,W = masks.shape
        scale= H//h
        masks_down = Fm.max_pool2d(masks, kernel_size=(scale, scale))
        masks_i = []
        # 对于每个索引i，计算除了第i个mask之外所有masks的按元素逻辑非，并将结果存储在masks_i[i]中
        first_hidden_i =[]
        for i,mask in enumerate(masks_down):
            # 使用torch.stack将所有masks（除了第i个）堆叠起来，并在第一个维度上扩展第i个mask以匹配形状  
            other_masks = torch.stack([m if j != i else torch.zeros_like(m) for j, m in enumerate(masks_down)], dim=0).sum(dim=0)
            other_masks=other_masks.cpu().numpy().astype(bool) 
            # 对堆叠的masks取逻辑非（即1 - mask，但在布尔上下文中是~mask），然后对所有masks进行按元素逻辑或  
            masks_i.append(~other_masks)
            masks_i[i] = torch.from_numpy(masks_i[i]).float()
            mask= masks_i[i].cpu().numpy()
            mask_save = mask.squeeze(0)*255
            cv2.imwrite('/nvme-ssd/yanyi/code/modify_code/ConsistI2V_Grounded-Segment/dilate_mask2/other_masks.png', mask_save)
            first_hidden = first_hidden * masks_i[i].to(first_hidden.device)
            first_hidden_i.append(first_hidden)
        first_hidden = first_hidden_i    
        # text_embeddings : torch.Size([2, 77, 1024])
        index_i =[]
        text_embeddings_i=[] 
        for i,index in enumerate(index_list):
            if index_list[i+1] is not None and isinstance(index_list[i+1], list):
                index_i.append( [index[0],index_list[i+1][0]])   
                text_embeddings_i.append(text_embeddings[:,index[0]:index_list[i+1][0],:]) 
            else:
                index_i.append([index_list[0],index_list[i+1]])
                text_embeddings_i.append(text_embeddings[:,index[0]:index_list[i+1],:]) 
                break    
    return first_hidden,text_embeddings_i
    
def show_wise_hidden(masks,image_path,device,vae,prompt,negative_prompt,img_transform,encode_prompt):
    if masks is not None :
        # 对于每个索引i，计算除了第i个mask之外所有masks的按元素逻辑非，并将结果存储在masks_i[i]中
        first_hidden_i =[]
        for i,mask in enumerate(masks):
            ...
            # img = image * mask.type_as(image) + ones_tensor * inverse_mask.type_as(image) 
            image = torch.from_numpy(np.array(Image.open(image_path).convert('RGB'))).permute(2, 0, 1).float()
            image = np.uint8((image*mask).permute(1, 2, 0).numpy())
            first_frame = img_transform(image)
            first_frames = first_frame.to(device, dtype=vae.dtype).unsqueeze(0)
            first_frame_latents = vae.encode(first_frames).latent_dist
            first_frame_latents = first_frame_latents.sample()
            first_frame_latents = first_frame_latents * vae.config.scaling_factor # b, c, h, w
            first_frame_latents_input = torch.cat([first_frame_latents] * 2).unsqueeze(2)
            first_hidden_i.append(first_frame_latents_input)
        first_hidden = first_hidden_i    
        
        # text_embeddings : torch.Size([2, 77, 1024])
        prompt_list=prompt[0].split(",")
        text_embeddings_i=[]
        for prompts in prompt_list:
            text_embeddings = encode_prompt(
                prompt=prompts, device=device, num_videos_per_prompt=1, do_classifier_free_guidance='text', negative_prompt=negative_prompt[0]
            )
            text_embeddings_i.append(text_embeddings) 
    return first_hidden,text_embeddings_i
   
def get_unet_cross_and_tempo_attn(unet):
    
    def get_attn(blocks=None,block_type=None):
        if block_type == 'mid_block':
            attn=blocks.attentions[0].transformer_blocks[0].attn2.cross_atten
            cross_attn_name= f"{block_type}_cross_attn_0_0"
            cross_tempo_atten[cross_attn_name] = attn 
        else:
            for i, block in enumerate(blocks):
                if hasattr(block, 'attentions'):
                    for j in range(len(block.attentions)):
                        attn=block.attentions[j].transformer_blocks[0].attn2.cross_atten
                        tempo_attn=block.tempo_attns[j].transformer_blocks[0].attn2.tempo_attns
                        cross_attn_name= f"{block_type}_cross_attn_{i}_{j}"
                        tempo_attn_name= f"{block_type}_tempo_attn_{i}_{j}"
                        cross_tempo_atten[cross_attn_name] = attn 
                        cross_tempo_atten[tempo_attn_name] = tempo_attn 
    cross_tempo_atten={}
    for name, module in unet.named_children():
        if 'down_blocks' in name :
            blocks = getattr( unet, name)
            block_type = 'down_blocks'
            get_attn(blocks,block_type=block_type)
