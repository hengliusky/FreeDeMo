...
class Transformer2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            if not self.use_linear_projection: # use_linear_projection = True
        ...
        if self.alpha is not None:# True 
            with torch.no_grad():
...
class BasicConditionalTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if condition_on_first_frame: # spatial attn
        ...
        elif self.is_temporal and self.augment_temporal_attention: # temporal-attn
            first_frame_hidden_states = rearrange(norm_hidden_states, '(b f) d h -> b f d h', f=self.n_frames)[:, 0, :, :]
            ...
            attn_output = self.attn1(
                norm_hidden_states,
                encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,# this encoder_hidden_states is None
                attention_mask=attention_mask,
        ...
        if self.attn2 is not None:
            ...
            if  (self.is_temporal==False) :
                if masks is None:
                    ...
                    attn_output = Decoupled_Attention(self.attn2,
                                                        norm_hidden_states,
                                                        encoder_hidden_states=encoder_hidden_states,
                                                        attention_mask=encoder_attention_mask,
                                                        masks=masks,
...
def Decoupled_Attention(attn,
        ...
        # head_to_batch_dim : Reshape the tensor from `[batch_size, seq_len, dim]` to `[batch_size * heads, seq_len, dim // heads],heads=5
        query = attn.head_to_batch_dim(query) 
        key = attn.head_to_batch_dim(key)
        value = attn.head_to_batch_dim(value)
        attention_probs = attn.get_attention_scores(query, key, attention_mask) # α*(q@k) + β*attn_mask
        # original_hidden_states = torch.bmm(attention_probs, value)
        atten_heads = attn.heads
        # masks 
        if masks is not None:    
            h = w = int(math.sqrt(attention_probs.shape[1]))
            _,_,H,W = masks.shape
            scale= H//h
            masks_down = F.max_pool2d(masks, kernel_size=(scale, scale))
            masks_down = rearrange(masks_down,'b s h w -> b s (h w)')

            sum_masks= sum(masks_down).to(query.device)
            hidden_list =[]
            for i in range(len(text_embeddings_list)):
                new_query= query*masks_down[i].unsqueeze(-1)
                new_key=attn.to_k(text_embeddings_list[i],)
                new_value=attn.to_v(text_embeddings_list[i],)
                new_key = attn.head_to_batch_dim(new_key)
                new_value = attn.head_to_batch_dim(new_value)
        
                attentions = attn.get_attention_scores(new_query, new_key, attention_mask)
                hidden=torch.bmm(attentions ,new_value) * masks_down[i].unsqueeze(-1)
                
                hidden_list.append(hidden)
                   
            
            hidden_states = sum(hidden_list).transpose(1, 2)/(sum_masks.unsqueeze(0))
        ...
        attn.batch_attention_value = attn.batch_to_head_dim(attention_probs)
        attn.attention_value = attention_probs
        
        # softmax(qk)v
        # batch_to_head_dim : Reshape the tensor from `[batch_size, seq_len, dim]` to `[batch_size // heads, seq_len, dim * heads]
        hidden_states = attn.batch_to_head_dim(hidden_states) 
        # attn.cross_atten = hidden_states
        # linear proj
        hidden_states = attn.to_out[0](hidden_states,)
        # dropout
        hidden_states = attn.to_out[1](hidden_states)
        attn.cross_attention_value = hidden_states
