...
class VideoLDMUNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # mid
        if mid_block_type == "UNetMidBlock2DCrossAttn": # true
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            encoder_hidden_states = repeat(encoder_hidden_states, 'b n c -> (b f) n c', f=video_length) # 1 77 1024 -> 16 77 1024
        # update text_embeddings_list
        split_text_embeddings_list=[]
        if text_embeddings_list is not None:
            for i in range(len(text_embeddings_list)):
                split_text_embeddings_list.append(repeat(text_embeddings_list[i], 'b n c -> (b f) n c', f=video_length)) # 1 77 1024 -> 16 77 1024
        
        # all_masks = masks.sum(dim=0)
        # all_masks.to(sample.device)
        # all_masks = F.resize(all_masks, (sample.shape[-2])).unsqueeze(0).unsqueeze(0)
        # # all_masks = torch.cat((torch.zeros_like(all_masks),all_masks),dim=0).repeat(1,1,sample.shape[2],1,1)
        # all_masks = all_masks.repeat(2,1,sample.shape[2],1,1)
        # sample = torch.cat((sample,all_masks),dim=1)
        ...
        default_overall_up_factor = 2**self.num_upsamplers # 2**3==8
        ...
        if self.use_frame_stride_condition:# True
        ...
        if self.config.addition_embed_type == "text": # self.config.addition_embed_type = None
        ...
        if self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_proj":# self.encoder_hid_proj=None
        ...
        # 2. pre-process
        sample = self.conv_in(sample) # C : 4->320
        # sample = self.conv_in_2(sample) # C : 5->320
        # sample_wise = show_wise_index_hidden(sample)

        # 2.5 GLIGEN position net     cross_attention_kwargs:None
        ...
        # 3. down
        # lora_scale=1.0,is_controlnet=false,is_adapter=false
        ...
        for i,downsample_block in enumerate( self.down_blocks):
            down_cross_tempo_atten = None
            if cross_tempo_atten_list is not None:
            
                down_cross_tempo_atten=[]
                for cross_tempo_atten in cross_tempo_atten_list:
                    sub={}
                    for key, value in cross_tempo_atten.items():
                        # block_type, attn_type, key_i, key_j = key.split("_")
                        block_type,_,_,_, key_i, key_j = key.split("_")
                        key_i = int(key_i)
                        if key_i == i and block_type=='down':    
                            sub[key] = value
                    down_cross_tempo_atten.append(sub)
                        
            # has_cross_attention=hasattr(downsample_block, "has_cross_attention")
            # print("has_cross_attention:::",has_cross_attention)
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(
                    ...
                    masks=masks,
                    text_embeddings_list=split_text_embeddings_list,
                    down_cross_tempo_atten =down_cross_tempo_atten,
                    **additional_residuals,
                )
        ...
        if self.mid_block is not None:
            mid_cross_atten = None
            if cross_tempo_atten_list is not None:
                mid_cross_atten=[]
                for cross_tempo_atten in cross_tempo_atten_list:
                    for key, value in cross_tempo_atten.items():
                        if 'corss' in key: 
                            mid_cross_atten.append(cross_tempo_atten[key])
            sample = self.mid_block(
                ...
                masks=masks,
                text_embeddings_list=split_text_embeddings_list,
                mid_cross_atten=mid_cross_atten,
            )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            up_cross_tempo_atten=None
            if cross_tempo_atten_list is not None:
                up_cross_tempo_atten=[]
                for cross_tempo_atten in cross_tempo_atten_list:
                    sub={}
                    for key, value in cross_tempo_atten.items():
                        # block_type, attn_type, key_i, key_j = key.split("_")
                        block_type,_,_,_, key_i, _ = key.split("_")
                        key_i = int(key_i)
                        if key_i == i and block_type=='up':    
                            sub[key] = value
                    up_cross_tempo_atten.append(sub)
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                sample = upsample_block(
                    ...
                    masks=masks,
                    text_embeddings_list=split_text_embeddings_list,
                    up_cross_tempo_atten=up_cross_tempo_atten,
                )
            else:
                sample = upsample_block(
                    hidden_states=sample,
    ...
    def parallel_predict_noise(sample=None):
