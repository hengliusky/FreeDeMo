...
class VideoLDMCrossAttnDownBlock(nn.Module):
    ...
    def forward(
        ...
        for j, (resnet, conv3d, attn, tempo_attn) in enumerate(zip(self.resnets, self.conv3ds, self.attentions, self.tempo_attns)):
            if down_cross_tempo_atten is not None:
                cross_attn_list=[]
                tempo_attn_list=[]
                for cross_tempo_atten in down_cross_tempo_atten:
                    for key, value in cross_tempo_atten.items():
                        _,_,_,_,_, key_j = key.split("_")
                        key_j = int(key_j)
                        if key_j == j and 'cross' in key:
                            cross_attn_list.append(cross_tempo_atten[key]) 
                        elif key_j == j and 'tempo' in key:
                            tempo_attn_list.append(cross_tempo_atten[key])
                cross_attn_sum=  sum(cross_attn_list)
                tempo_attn_sum= sum(tempo_attn_list)
            ...
            hidden_states = attn(
                ...
                condition_on_first_frame=condition_on_first_frame,# true
                masks=masks,
                text_embeddings_list=text_embeddings_list,
                attn_sum = cross_attn_sum if down_cross_tempo_atten is not None else None
            ).sample
            hidden_states = tempo_attn(
                ...
                masks=masks,
                text_embeddings_list=text_embeddings_list,
                attn_sum = tempo_attn_sum if down_cross_tempo_atten is not None else None
            ).sample
...
class VideoLDMCrossAttnUpBlock(nn.Module):
    ...
    def forward(
        ...
        for j,(resnet, conv3d, attn, tempo_attn) in enumerate(zip(self.resnets, self.conv3ds, self.attentions, self.tempo_attns)):
            if up_cross_tempo_atten is not None:
                cross_attn_list=[]
                tempo_attn_list=[]
                for cross_tempo_atten in up_cross_tempo_atten:
                    for key, value in cross_tempo_atten.items():
                        _,_,_,_,_, key_j = key.split("_")
                        key_j = int(key_j)
                        if key_j == j and 'cross' in key:
            ...
            hidden_states = attn(
                ...
                masks=masks,
                text_embeddings_list=text_embeddings_list,
                attn_sum = cross_attn_sum if up_cross_tempo_atten is not None else None
            ).sample
            hidden_states = tempo_attn(
                hidden_states,
                encoder_hidden_states=encoder_hidden_states,
                ...
                masks=masks,
                text_embeddings_list=text_embeddings_list,
                attn_sum = tempo_attn_sum if up_cross_tempo_atten is not None else None
            ).sample
...
class VideoLDMUNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        for attn, resnet, conv3d in zip(self.attentions, self.resnets[1:], self.conv3ds[1:]):
            if mid_cross_atten is not None:
                mid_cross_atten_sum=sum(mid_cross_atten)
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = attn(
                    ...
                    masks=masks,
                    text_embeddings_list=text_embeddings_list,
                    attn_sum = mid_cross_atten_sum if mid_cross_atten is not None else None
...
class VideoLDMDownBlock(DownBlock2D):
    ...
    def forward(self, hidden_states, temb=None, scale: float = 1, first_frame_latents=None,masks=None):
        # input shape: hidden_states = (b f) c h w, first_frame_latents = b c 1 h w
        if self.first_frame_condition_mode == "conv2d": # self.first_frame_condition_mode== concat
            hidden_states = rearrange(hidden_states, '(b t) c h w -> b c t h w', t=self.n_frames)
            hidden_height = hidden_states.shape[3]
            first_frame_height = first_frame_latents.shape[3]
            downsample_ratio = hidden_height / first_frame_height
...
class VideoLDMUpBlock(UpBlock2D):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, upsample_size=None, scale: float = 1, first_frame_latents=None,masks=None):
        # input shape: hidden_states = (b f) c h w, first_frame_latents = b c 1 h w
        if self.first_frame_condition_mode == "conv2d":
            hidden_states = rearrange(hidden_states, '(b t) c h w -> b c t h w', t=self.n_frames)
            hidden_height = hidden_states.shape[3]
