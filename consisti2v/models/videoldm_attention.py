        ...
        **cross_attention_kwargs):
        ...
        if text_embeddings_list is not None:
            ...
            for i in range(len(text_embeddings_list)):
                ...
                num_difference = sum(text_embeddings_list[0]!=text_embeddings_list[1])
                text_embeddings = text_embeddings[::self.n_frames] 
                text_embeddings = repeat(text_embeddings, 'b n c -> (b hw) n c', hw=hw) 
                split_text_embeddings_list.append(text_embeddings)
                

        if adjacent_slices is not None: 
            assert encoder_hidden_states is None
            adjacent_slices = rearrange(adjacent_slices, 'b c h w n -> b (h w) n c')
            if not self.use_rotary_emb: # False
...
class RotaryEmbAttnProcessor2_0:
    ...
    def __call__(
        ...
        hw = int(residual.shape[0]/2)
        input_batch_size=int(residual.shape[0]/hw)
        ...
        if masks is not None:
            ...
            for text_embeddings in text_embeddings_list:
                ...
                sqlit_key = attn.to_k(text_embeddings, scale=scale)
                sqlit_value = attn.to_v(text_embeddings, scale=scale)
                sqlit_key = sqlit_key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) 
                sqlit_value = sqlit_value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2) 
                key_value_list.append(sqlit_key)
                value_list.append(sqlit_value)
                
            attention_output_list=[]
            # mask the rest of the subjects
            h = w = int(hw **0.5)
            _,_,H,W = masks.shape
            scale= H//h
            masks_down = F.max_pool2d(masks, kernel_size=(scale, scale))
            modify_masks = rearrange(masks_down,'b s h w -> b s (h w)').to(hidden_states.device)
            sum_new_masks = sum(modify_masks)
            
            for i in range(len(text_embeddings_list)):
                new_query=query.view(-1,hw,attn.heads,attn.n_frames,head_dim)
                query_mask=modify_masks[i].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                new_query=new_query*query_mask
                new_query = new_query.view(batch_size, attn.heads,-1, head_dim)
                new_key=key_value_list[i]
                new_value=value_list[i]
                scores = torch.matmul(new_query,new_key.transpose(-2,-1)) / torch.sqrt(torch.tensor(query.size(-1),dtype=query.dtype))
                attn_weights = F.softmax(scores,dim=-1)
                output_attn=torch.matmul(attn_weights, new_value)
                output_attn = rearrange(output_attn, '(b hw) n t c -> (b t) (n c) hw', hw=hw,b=input_batch_size,t=query.shape[2],)
                output_attn = output_attn*modify_masks[i].unsqueeze(0)
                output_attn=rearrange(output_attn, '(b t) (n c) hw -> (b hw) n t c',hw=hw,b=input_batch_size, t=query.shape[2],n=attn.heads,c=head_dim)
                
                attention_output_list.append(output_attn)
            
            attention_output=sum(attention_output_list)
            attention_output = rearrange(attention_output, '(b hw) n t c -> (b t) (n c) hw', hw=hw,b=input_batch_size,t=num_frames)
            attention_output = attention_output/(sum_new_masks.unsqueeze(0))
            attention_output=rearrange(attention_output, '(b t) (n c) hw -> (b hw) n t c',hw=hw,b=input_batch_size, t=num_frames,n=attn.heads,c=head_dim)
